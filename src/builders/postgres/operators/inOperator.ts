    ...
    static build(fieldType: FieldType, field: string, value: any, index: number): PostgresExpression | null {
        ...
        if (!isArray(value)) {
            if (fieldType === FieldType.FieldTypeString) {
                return new PostgresExpression(`LOWER(${field}) IN (LOWER($${index}))`, [value]);
            } else {
                return new PostgresExpression(`${field} IN ($${index})`, [value]);
            }
        }
        ...
        if (fieldType === FieldType.FieldTypeString) {
            ...
            return new PostgresExpression(`LOWER(${field}) IN (${indexes.join(",")})`, value);
        } else {
            const indexes = [];
            for (let i = index; i < index + value.length; i++) {
                indexes.push(`$${i}`);
            }
            return new PostgresExpression(`${field} IN (${indexes.join(",")})`, value);
        }
    }
}
